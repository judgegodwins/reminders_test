...
const createComponentLogger = (component) => (0, winston_1.createLogger)({
    level: 'debug',
    format: winston_1.format.combine(winston_1.format.timestamp({ format: () => new Date().toLocaleString() }), winston_1.format.errors({ stack: true }), winston_1.format.splat(), winston_1.format.json()),
